...
export default defineEventHandler(async (event) => {
  ...
  if (supabase) {
    try {
      const { data: profiles, error: pErr } = await supabase.from('profiles').select('plan, role, quota_used_this_month')
      if (!pErr && profiles) {
      ...
      console.warn('analytics tables not ready, using memory fallback')
    }
  }
