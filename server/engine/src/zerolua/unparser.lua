...
local SAFE_ALPHA = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
local function randomAlpha(len)
	if not len or len <= 0 then return "" end
	local res = {}
	for i = 1, len do
		local idx = math.random(1, #SAFE_ALPHA)
		table.insert(res, SAFE_ALPHA:sub(idx, idx))
	end
	return table.concat(res)
end

local function morphNumber(n)
	if n == 0 then return "(#\"\")" end
	if n < 0 then return "(-" .. morphNumber(-n) .. ")" end
	if math.floor(n) ~= n then
		local scaled = math.floor(n * 10000 + 0.5)
		return "(" .. morphNumber(scaled) .. "/(" .. morphNumber(10000) .. "))"
	end

	-- Small numbers 1..16: direct string length with randomized alphabetic characters
	if n <= 16 then
		return "(#\"" .. randomAlpha(n) .. "\")"
	end

	-- Numbers 17..255: base 16 decomposition: n = a * 16 + b (pure integer arithmetic, no float pow)
	if n <= 255 then
		local b16 = "(#\"" .. randomAlpha(16) .. "\")"
		local a = math.floor(n / 16)
		local b = n % 16
		local aStr = "(#\"" .. randomAlpha(a) .. "\")"
		local bStr = "(#\"" .. randomAlpha(b) .. "\")"
		if b == 0 then
			return "(" .. aStr .. "*" .. b16 .. ")"
		end
		if a == 0 then
		...
		return "((" .. aStr .. "*" .. b16 .. ")+" .. bStr .. ")"
	end

	-- Numbers >= 256: base 256 decomposition: n = q * 256 + r (pure integer arithmetic)
	local b16 = "(#\"" .. randomAlpha(16) .. "\")"
	local b256 = "((" .. b16 .. ")*(" .. b16 .. "))"
	local q = math.floor(n / 256)
	local r = n % 256
	if r == 0 then
		return "(" .. morphNumber(q) .. "*" .. b256 .. ")"
	end
	return "((" .. morphNumber(q) .. "*" .. b256 .. ")+" .. morphNumber(r) .. ")"
end
...
function Unparser:whitespaceIfNeeded(following, ws)
	local firstChar = string.sub(string.match(following, "^%s*(.*)") or "", 1, 1);
	if(self.prettyPrint or self.identCharsLookup[firstChar]) then
...
function Unparser:whitespaceIfNeeded2(leading, ws)
	...
	if(self.prettyPrint or self.identCharsLookup[lastChar] or self.numberCharsLookup[lastChar] or lastChar == ")" or lastChar == "]" or lastChar == "}" or lastChar == '"' or lastChar == "'") then
		return ws or self.SPACE;
	end
	return "";
end
...
function Unparser:unparseStatement(statement, tabbing)
	...
	elseif(statement.kind == AstKind.IfStatement) then
		...
		push(self:newline(false), self:whitespaceIfNeeded2(bodyCode, " "), "end");
	...
	elseif(statement.kind == AstKind.ReturnStatement) then
		...
		local operator = compoundOps[statement.kind]
		if operator then
			local lhsStr = self:unparseExpression(statement.lhs, tabbing);
			local rhsStr = self:unparseExpression(statement.rhs, tabbing);
			push(lhsStr, self:optionalWhitespace(), "=", self:optionalWhitespace(), lhsStr, self:optionalWhitespace(), operator, self:optionalWhitespace(), rhsStr);
		else
...
function Unparser:unparseExpression(expression, tabbing)
	...
	if(expression.kind == AstKind.NumberExpression) then
		...
		local numVal = expression.value;
		if numVal == math.huge or tostring(numVal) == "inf" then
			return "2e1024"
		end
		if numVal == -math.huge or tostring(numVal) == "-inf" then
			return "-2e1024"
		end
		local s = tostring(numVal);
