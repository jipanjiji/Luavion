...
local Tokenizer = require("zerolua.tokenizer");
...
local function generateError(self, message)
	...
	for offset = -5, 5 do
		...
		if idx >= 1 and idx <= self.length then
			local tk = self.tokens[idx]
			table.insert(ctx, string.format("[%d:%s:%s]", tk.line, tk.kind, tk.source or ""))
		end
	end

	return "Parsing Error at Position " .. tostring(token.line) .. ":" .. tostring(token.linePos) .. ", " .. message .. " Context: " .. table.concat(ctx, " ");
end
...
function Parser:statement(scope, currentLoop)
	...
	if expr then
		...
		if(expr.kind == AstKind.IndexExpression or expr.kind == AstKind.VariableExpression) then
			...
			-- Compound Assignment (Luau/Extended Syntax)
			if(consume(self, TokenKind.Symbol, "+=")) then
