...
function Tokenizer:getPosition(i)
	i = i or 1
	if i < 1 then i = 1 end
	local column = self.columnMap and (self.columnMap[i] or self.columnMap[#self.columnMap] or self.columnMap[1])

	if not column then
		return 1, i
	end

	return column.id or 1, (column.charMap and column.charMap[i]) or 1
end
