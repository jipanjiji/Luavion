...
local RandomLiterals = require("zerolua.randomLiterals")
...
function ProxifyLocals:apply(ast, pipeline)
    ...
    local function isSemanticSensitive(node)
        return node and (node.__hook_sensitive or node.__semantic_preserve or node.__do_not_touch or node.__ignoreProxifyLocals)
    end
    ...
    visitast(ast, function(node, data)
        ...
        -- Lock __do_not_touch and __semantic_preserve declarations
        if(node.kind == AstKind.LocalVariableDeclaration and isSemanticSensitive(node)) then
            for i, id in ipairs(node.ids) do
                disableMetatableInfo(node.scope, id);
            end
        end
        if(node.kind == AstKind.LocalFunctionDeclaration and isSemanticSensitive(node)) then
            disableMetatableInfo(node.scope, node.id);
        end
        ...
        if(node.kind == AstKind.AssignmentStatement) then
            if(#node.lhs == 1 and node.lhs[1].kind == AstKind.AssignmentVariable) then
                ...
                if isSemanticSensitive(node) or isSemanticSensitive(variable) or isSemanticSensitive(node.rhs[1]) then
        ...
        if(node.kind == AstKind.LocalVariableDeclaration) then
            if isSemanticSensitive(node) then
                for i, id in ipairs(node.ids) do
                    disableMetatableInfo(node.scope, id);
                end
                return node;
        ...
        -- Variable Expression
        if(node.kind == AstKind.VariableExpression and not isSemanticSensitive(node)) then
        ...
        if(node.kind == AstKind.AssignmentVariable and not isSemanticSensitive(node)) then
            local localMetatableInfo = getLocalMetatableInfo(node.scope, node.id);
            -- Apply Only to Some Variables if Threshold is non 1
            if localMetatableInfo then
                return Ast.AssignmentIndexing(Ast.VariableExpression(node.scope, node.id), Ast.StringExpression(localMetatableInfo.valueName));
            end
        end
