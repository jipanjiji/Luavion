...
function SplitStrings:apply(ast, pipeline)
	...
	visitAst(ast, function(node, data)
		...
		if(node.kind == Ast.AstKind.StringExpression) then
			...
			if data and data.parent and (data.parent.kind == Ast.AstKind.KeyedTableEntry and data.key == "key"
				or ((data.parent.kind == Ast.AstKind.IndexExpression or data.parent.kind == Ast.AstKind.AssignmentIndexing) and data.key == "index")) then
				return
			end
			local str = node.value;
			if not str or string.len(str) < math.max(6, self.MinLength * 2) then
				return
			end
			local chunks = {};
			local i = 1;
