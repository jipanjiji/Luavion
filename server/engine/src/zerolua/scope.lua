...
function Scope:renameVariables(settings)
	if(not self.isGlobal) then
		...
		local current = self.parentScope;
		while current and not current.isGlobal do
			for id, varName in pairs(current.variables) do
				if not current.skipIdLookup[id] then
					forbiddenNamesLookup[varName] = true;
				end
			end
