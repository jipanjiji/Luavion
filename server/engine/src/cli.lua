...
-- cli.lua
--
-- This Script contains the Code for the Zero Impact CLI.

-- Configure package.path for requiring Zero Impact.
...
local INSTALL_SCRIPT_URL = "https://raw.githubusercontent.com/FakeModz/Zero-Obfuscator/main/install.sh"
...
local function get_version()
	local version = os.getenv("ZeroLua_LUA_VERSION")
...
local function print_help()
	print("ZeroLua Lua CLI")
	print("Usage: ZeroLua-lua [command] [options] <input.lua>")
	print("")
	print("Commands:")
	...
	print("  --seed, --s <num>      Custom seed (default: random high-entropy)")
...
local function run_update()
	...
	print("Updating ZeroLua-lua using official installer")
...
local ZeroLua = require("zerolua")
ZeroLua.Logger.logLevel = ZeroLua.Logger.LogLevel.Info
ZeroLua.Logger.errorCallback = function(...)
	local args = { ... }
	local message = table.concat(args, " ")
	io.stderr:write(ZeroLua.colors(ZeroLua.Config.NameUpper .. ": " .. message, "red") .. "\n")
	os.exit(1)
end
...
local function run_cli()
	-- CLI
	local config, sourceFile, outFile, luaVersion, prettyPrint, customSeed

	ZeroLua.colors.enabled = true
	...
	while i <= #arg do
		...
		if curr:sub(1, 2) == "--" then
			if curr == "--seed" or curr == "--s" then
				i = i + 1
				customSeed = tonumber(arg[i])
			elseif curr == "--preset" or curr == "--p" then
				if config then
					ZeroLua.Logger:warn("The config was set multiple times")
				end

				i = i + 1
				local preset = ZeroLua.Presets[arg[i]]
				if not preset then
					ZeroLua.Logger:error(string.format('A Preset with the name "%s" was not found!', tostring(arg[i])))
				end
			...
			elseif curr == "--config" or curr == "--c" then
				...
				if not file_exists(filename) then
					ZeroLua.Logger:error(string.format('The config file "%s" was not found!', filename))
				end
				...
				if not func then
					ZeroLua.Logger:error(string.format('Failed to parse config file "%s": %s', filename, tostring(err)))
				end
				config = func()
			elseif curr == "--out" or curr == "--o" then
				i = i + 1
				if outFile then
					ZeroLua.Logger:warn("The output file was specified multiple times!")
				end
				outFile = arg[i]
			elseif curr == "--nocolors" then
				ZeroLua.colors.enabled = false
			...
			elseif curr == "--saveerrors" then
				...
				ZeroLua.Logger:warn(string.format('The option "%s" is not valid and therefore ignored', curr))
			end
		else
			if sourceFile then
				ZeroLua.Logger:error(string.format('Unexpected argument "%s"', arg[i]))
			end
	...
	if not sourceFile then
		ZeroLua.Logger:error("No input file was specified!")
	end

	if not config then
		ZeroLua.Logger:warn("No config was specified, falling back to Minify preset")
		config = ZeroLua.Presets.Minify
	end
	...
	if not file_exists(sourceFile) then
		ZeroLua.Logger:error(string.format('The File "%s" was not found!', sourceFile))
	end
	...
	local pipeline = zerolua.pipeline:fromConfig(config)
	local out = pipeline:apply(source, sourceFile)
	ZeroLua.Logger:info(string.format('Writing output to "%s"', outFile))
