...
export function usePlans() {
  const currency = useState<CurrencyCode>('currency_pref', () => 'USD')
  ...
  const formatUnitRate = (usd: number, idr: number) => {
    if (currency.value === 'IDR') {
  ...
  return {
    plans: PLANS,
    topUpPricing: TOP_UP_PRICING,
    currency,
    formatPrice,
    formatUnitRate,
    getPlan: (id: string = 'free') => PLANS[(id || 'free').toLowerCase() as PlanTier] || PLANS.free
  }
}
